% Created 2024-02-10
% function for fitting thermal hysteresis measurements without lower
% baselines, but with evidence of proximal lower baselines

function rss = meritNoLowBaseline(parameters, stages, Tref, plotI, varargin)

simulation = {};
y0 = [1,0];

% 30 minutes holding at high temperature (95 degrees)
tspan = 0:1:30;
...
slopesAndIntercepts = linsolve(A, Y);
rss = 0;
absDiffArr = varargin{3};
x = varargin{4};
for j = 1:length(simulation)
    mHigh = x(2);
    bHigh = x(2+j);
    mLow = x(1);
    %bLow = x(1+j)-varargin{3};
    absDiff = absDiffArr(j);
    if mod(j,2) == 1    % cooling
        highBaselineAbs = stages{j}.tempArr(end) * mHigh + bHigh;
    else                % heating
        highBaselineAbs = stages{j}.tempArr(1) * mHigh + bHigh;
    end
    lowBaselineAbs = highBaselineAbs - absDiff;
    if mod(j,2) == 1    % cooling
        bLow = lowBaselineAbs - stages{j}.tempArr(end) * mLow;
    else                % heating
        bLow = lowBaselineAbs - stages{j}.tempArr(1) * mLow;
    end
    ...
    simu = simulation{j}(:,1);
    rss = rss + sum((normalizedAbs'-simu).^2);
end
%rss = sum((A*x-Y).^2);
...
if plotI ~= -1
    ...
    simulatedY = A*slopesAndIntercepts;
    simulatedAbsCell = {};
    index = 1;
    for j = 1:length(simulation)
    ...
    deltaAbs = deltaAbs/length(simulation);
